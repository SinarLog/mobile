...
import { getMyEmployeesLeave } from "../../../Network/EmployeeFlow/RemoteStorage"
...
const LeaveView = ({ id }) => {
    ...
    const loadData = async () => {
        try {
            const data = await getMyEmployeesLeave(id)
            if (data) {
    ...
    const loadDataWithFilter = async () => {
        try {
            const data = await getMyEmployeesLeave(id, selectedStatus, valueMonth, valueYear, selectedOrder)
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <Pressable className="bg-white mb-2 rounded"> 
                        ...
                        <View className="flex-row items-center py-2 justify-end px-4">
                ...
                ListEmptyComponent={ () => (
                    <View className="bg-white py-4 items-center justify-center rounded">
                        <Image source={NoDataIcon}/>
                        {
                            ...
                            :
                            <Text className="text-textHitam text-xs font-semibold">Your employee doesnt have Leave History</Text>
                        }
                    </View>
                )}
            />
