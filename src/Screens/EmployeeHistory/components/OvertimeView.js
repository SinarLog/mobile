...
import { getMyEmployeesOvertime } from "../../../Network/EmployeeFlow/RemoteStorage"

const OvertimeView = ({ id }) => {
    const [overtimeEmployee, setOvertimeEmployee] = useState([])
    ...
    const loadData = async () => {
        try {
            const data = await getMyEmployeesOvertime(id)
            ...
            if (data) {
                ...
                setOvertimeEmployee(updatedData)
            }
        } catch (error) {
    ...
    const loadDataWithFilter = async () => {
        try {
            const data = await getMyEmployeesOvertime(id, selectedStatus, selectedOrder)
            if (data) {
                const updatedData = data.map(item => {
                    if (item.status === 'PENDING') {
                        return {
                ...
                ListEmptyComponent={ () => (
                    <View className="bg-white py-4 items-center justify-center rounded">
                        <Image source={NoDataIcon}/>
                        {
                            ...
                            :
                            <Text className="text-textHitam text-xs font-semibold">This Employee doesnt have overtime History</Text>
                        }
                    </View>
                )}
            />
