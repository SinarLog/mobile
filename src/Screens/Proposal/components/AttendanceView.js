...
import { getStaffAttendanceHistory } from "../../../Network/ApprovalFlow/RemoteStorage"
...
const AttendanceView = () => {
    ...
    const loadData = async () => {
        try {
            const data = await getStaffAttendanceHistory(9999999)
            if (data) {
    ...
    const loadDataWithFilter = async () => {
        try {
            const data = await getStaffAttendanceHistory(9999999, value, selectedStatus, valueMonth, valueYear)
            if (data) {
                setAttendanceStaff(data)
            }
            else {
